...
>(({ className, showIcon = false, ...props }, ref) => {
  // Use a stable identifier for consistent width
  const stableId = React.useId();

  // Random width between 50 to 90%.
  const width = React.useMemo(() => {
    // Use a stable hash based on component instance for consistency
    const hash =
      (stableId || 'skeleton').split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 40;
    return `${hash + 50}%`;
  }, [stableId]);
