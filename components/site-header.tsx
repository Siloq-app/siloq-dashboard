...
const automationModes = [
  ...
  { id: 'semi' as const, label: 'Semi-Auto', desc: 'Safe changes auto-approved' },
  { id: 'full' as const, label: 'Full Auto', desc: 'All changes executed automatically' },
]
...
export function SiteHeader({ 
  ...
  return (
    <header className="flex h-16 shrink-0 items-center gap-2 border-b bg-background transition-[width,height] ease-linear px-4 lg:px-6">
      <div className="flex w-full items-center gap-1 lg:gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            ...
            >
              <span className="hidden sm:inline text-xs text-gray-600 dark:text-gray-400">Automation</span>
              <span className="font-medium">{automationModes.find(m => m.id === automationMode)?.label}</span>
              ...
              <div className="absolute right-0 top-full z-50 mt-1 w-56 rounded-lg border bg-white p-1 shadow-lg">
                {automationModes.map((mode) => (
                  ...
                  >
                    ...
                    <span className={automationMode === mode.id ? "ml-0" : "ml-6"}>{mode.label}</span>
                  </button>
                ))}
              </div>
            )}
